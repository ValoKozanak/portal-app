...
// Použitie: node import_pohoda_xml.js
const fileName = process.argv[2] || 'VydFaktury.xml';
const companyId = process.argv[3] || 3;

console.log(`🚀 Spúšťam import súboru: ${fileName} pre company_id: ${companyId}`);

importPohodaXml(fileName, companyId)
  .then(result => {
    console.log('\n🎉 Import úspešne dokončený!');
    console.log(`📊 Celkovo importovaných: ${result.importedCount} faktúr`);
    db.close();
  })
  .catch(error => {
