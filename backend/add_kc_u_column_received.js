...
async function addKcUColumnReceived() {
  try {
    console.log('🔧 Pridávam stĺpec kc_u do tabuľky received_invoices...');
    
    // Pridanie stĺpca kc_u
    db.run(`ALTER TABLE received_invoices ADD COLUMN kc_u DECIMAL(10,2) DEFAULT 0`, (err) => {
      if (err) {
