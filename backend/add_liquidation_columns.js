...
async function addLiquidationColumns() {
  try {
    console.log('🔧 Pridávam stĺpce pre likvidáciu do tabuľky issued_invoices...');
    ...
    for (const column of columns) {
      db.run(`ALTER TABLE issued_invoices ADD COLUMN ${column.name} ${column.type}`, (err) => {
        if (err) {
