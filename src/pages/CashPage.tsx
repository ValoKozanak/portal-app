...
import { accountingService, CashTransaction } from '../services/accountingService';
...
const CashPage: React.FC = () => {
  ...
  const loadTransactions = async () => {
    try {
      setLoading(true);
      const data = await accountingService.getCashTransactions(companyId, { limit: 100 });
      setTransactions(data);
    } catch (error) {
      console.error('Chyba pri načítaní pokladničných transakcií:', error);
    } finally {
      setLoading(false);
    }
  };
  ...
  const getTypeBadge = (type: string) => {
    const typeConfig = {
      income: { color: 'bg-green-100 text-green-800', text: 'Príjem' },
      expense: { color: 'bg-red-100 text-red-800', text: 'Výdavok' },
  ...
  const handleDeleteTransaction = async (transaction: CashTransaction) => {
    ...
    try {
      await accountingService.deleteCashTransaction(transaction.id);
      ...
      <div className="bg-white dark:bg-dark-800 shadow-sm border-b border-gray-200 dark:border-gray-700">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            ...
            <div className="flex items-center">
              <h1 className="text-xl font-semibold text-gray-900 dark:text-white">Pokladňa</h1>
            </div>
          </div>
        </div>
      </div>
          ...
          <div className="px-6 py-4 border-b border-gray-200 dark:border-gray-700">
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-4">
                <h2 className="text-lg font-semibold text-gray-900 dark:text-white">Pokladničné transakcie</h2>
