...
const EmailTestModal: React.FC<Props> = ({ isOpen, onClose }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const resp = await fetch(`${API_BASE}/test/send-test-email`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', ...authHeaders() },
        body: JSON.stringify({ email, type: emailType }),
      });
      const data = await resp.json();
      if (resp.ok) setResult({ success: true, message: data.message });
      else setResult({ success: false, message: data.error || 'Neznáma chyba' });
    } catch (error: any) {
      setResult({ success: false, message: error?.message || 'Chyba pri odosielaní požiadavky' });
    } finally {
      setIsLoading(false);
    }
  };
      ...
      <div className="bg-white rounded-lg shadow-xl max-w-md w-full mx-4 max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          ...
          <div>
            ...
            <p className="mt-1 text-sm text-gray-500">{emailTypes.find((t) => t.value === emailType)?.description}</p>
          </div>

          {result && (
            <div className={`p-4 rounded-md ${result.success ? 'bg-green-50 border border-green-200' : 'bg-red-50 border border-red-200'}`}>
              <div className="flex items-center space-x-2">
                {result.success ? <CheckCircleIcon className="h-5 w-5 text-green-600" /> : <ExclamationCircleIcon className="h-5 w-5 text-red-600" />}
          ...
          <div className="flex space-x-3 pt-4">
            <button type="button" onClick={handleClose} className="flex-1 px-4 py-2 border border-gray-300 rounded-md text-gray-700 hover:bg-gray-50 transition-colors">
              Zrušiť
            </button>
            <button type="submit" disabled={isLoading} className="flex-1 px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors">
