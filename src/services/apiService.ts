// =========================
// Base URL (ENV) – jednotný zdroj pravdy
// =========================
// Používame REACT_APP_API_BASE, inak default /api (Netlify proxy).
const API_BASE_URL: string = (process.env.REACT_APP_API_BASE || '/api').replace(/\/$/, '');
...
class ApiService {
  ...
  setToken(token: string) {
    ...
    localStorage.setItem('auth_token', token); // kompatibilita, ak niekde číta auth_token
  }
  ...
  async postFormData<T>(endpoint: string, formData: FormData): Promise<T> {
    return this.request<T>(endpoint, {
      method: 'POST',
      body: formData, // Content-Type nech prehliadač
    });
  }
  ...
  async updateUser(id: number, userData: {
    name: string; email: string; role: string; status?: string; phone?: string;
  }) {
  ...
  async deleteUser(id: number) {
    return this.request<{ message: string }>(`/auth/users/${id}`, { method: 'DELETE' });
  }
  ...
  async deleteCompany(id: number) {
    return this.request<{ message: string }>(`/companies/${id}`, { method: 'DELETE' });
  }
  async deactivateCompany(id: number) {
    return this.request<{ message: string }>(`/companies/${id}/deactivate`, { method: 'PATCH' });
  }
  async activateCompany(id: number) {
    return this.request<{ message: string }>(`/companies/${id}/activate`, { method: 'PATCH' });
  }
  ...
  // Tasks endpoints
  // =========================
  async getAllTasks() { return this.request<Task[]>('/tasks'); }
  async getCompanyTasks(companyId: number) { return this.request<Task[]>(`/tasks/company/${companyId}`); }
  ...
  async getTask(taskId: number) { return this.request<Task>(`/tasks/${taskId}`); }
  ...
  async deleteTask(id: number) { return this.request<{ message: string }>(`/tasks/${id}`, { method: 'DELETE' }); }

  // =========================
  // Files endpoints
  // =========================
  async getAllFiles() { return this.request<FileData[]>('/files/admin/all'); }
  async getCompanyFiles(companyId: number) { return this.request<FileData[]>(`/files/company/${companyId}`); }
  ...
  async getAllMessages() { return this.request<any[]>('/messages/admin/all'); }
  ...
  async getAllDocuments() { return this.request<DocumentData[]>('/documents/admin/all'); }
